...
@torch.no_grad()
def evaluate_checkpoint(ckpt_path, data_by_symbol, encoder_ckpt, feature_cols, device):
    SEED = 12345
    import random
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    action_masks = extract_action_masks(infos, device)
    ...
    while not done.all():
        ...
        actions, _, _ = policy.act(latent, action_masks, deterministic=True)
...
def main():
    base_dir = "backend/artifacts/phase_d/mask_on/seed_0"
    ...
    while True:
        ...
        for ckpt in ckpts:
            ...
            # Save per-step equity curves for later analysis/plotting
            curve_rows = []
            for sym, c in zip(symbols, curves):
                for t, eq in enumerate(c):
                    curve_rows.append({"symbol": sym, "t": t, "equity": float(eq)})
            pd.DataFrame(curve_rows).to_csv(
                os.path.join(log_dir, ckpt.replace(".pt", "_equity.csv")),
                index=False,
            )
